...
- `Dockerfile` and CI workflow: **Docker integration tests** run `pytest tests/integration` inside the built image (host networking to Qdrant/Ollama); the venv job runs **unit and integration** tests in one `pytest` with coverage; **GHCR** push runs only after lint, security, that test job, and Docker tests succeed, on **main** and version tags (`v*`).
